...
static void watch_update()
{
	...
	uint16_t prog=data.step>10000?10000:data.step;
	prog=4999*128/10000;
	//prog=last_prog+10;
	...
	GPoint prog_bar_points[4] = { {0,0},{prog,0},{prog,5},{0,5}};
	Polygon prog_bar = {4, prog_bar_points};
	Geometry geometry = {GeometryTypePolygon, FillArea,GColorBlack,(void*)&prog_bar}; 
	p_geometry_array[0] = &geometry;
	
	/*直线*/
	Line l = {{0,6}, {128,6}};	//直线的起始点坐标

	/*几何结构体，依次为几何类型、填充类型、填充颜色、具体的几何图结构*/
	Geometry lg = {GeometryTypeLine, FillOutline, GColorBlack, (void*)&l}; 
	p_geometry_array[1] = &lg;	//存入指针数组中
	
	LayerGeometry prog_bar_struct = {2, p_geometry_array};
