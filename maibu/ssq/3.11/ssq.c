...
static GRect timeframe={{0,8},{20,128}};
static GRect text1frame={{0,50},{20,128}};
static GRect text2frame={{0,70},{20,128}};
static GRect text3frame={{0,90},{20,128}};




/*图层句柄*/
static uint16_t g_layer_time=-1,g_layer_text1=-1,g_layer_text2=-1,g_layer_text3=-1;
...
void web_recv_callback(const uint8_t *buff, uint16_t size)
{
	...
	P_Layer p_layer_temp = app_window_get_layer_by_id(p_window, g_layer_text1);
    ...
    p_layer_temp = app_window_get_layer_by_id(p_window, g_layer_text2);
...
static P_Window init_watch(void)
{
	...
	sprintf(buff,"双色球开奖结果");
    LayerText timetext = {buff, timeframe, GAlignCenter,U_ASCII_ARIAL_12};//
	...
	sprintf(buff,"----");
    LayerText text1 = {buff, text1frame, GAlignLeft,U_ASCII_ARIAL_12};//
	...
	sprintf(buff,"----");
	LayerText text3 = {buff, text3frame, GAlignLeft,U_ASCII_ARIAL_12};//
