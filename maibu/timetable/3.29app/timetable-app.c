...
P_Window init_window(void)
{
	...
	if(app_persist_get_data_size(TIME_KEY)==0) 
	{
		...
		tmpframe.size.w=128;
		sprintf(tmp,"请先设置课程表");
		display_target_layerText(p_window,&tmpframe,GAlignCenter,GColorWhite,tmp,U_ASCII_ARIAL_12);
		return p_window;
	}
...
//	{
	...
	//绘制表格//行宽和行高   4列，最多10行
	int x0=4,y0=12,w=40,h;
	h=classcount1>9?12:116/classcount1;
