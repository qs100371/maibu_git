...
P_Window init_window(void)
{
	...
	//添加背景图层
	GRect bgframe = {{0,0},{128,128}};
	display_target_layer(p_window,&bgframe,GAlignCenter,GColorWhite,bmp_array_name,10);

	//添加日期周文本图层
	GRect ymdframe = {{18,4},{12,92}};
	display_target_layerText(p_window,&ymdframe,GAlignCenter,GColorWhite,g_str_ymd,U_ASCII_ARIAL_12);

	//添加时间图片图层
	GRect timeframe = {{23,27},{34,19}};

	uint8_t ii;
	for(ii = 0; ii <=3 ; ii++)
	{
		display_target_layer(p_window,&timeframe,GAlignLeft,GColorWhite,bmp_array_name,g_time_bmp_num[ii]);
		timeframe.origin.x = timeframe.origin.x + 19;
		if(ii == 1)
		{
			timeframe.origin.x = timeframe.origin.x + 9;
		}
	}
	/*添加按键事件*/
	...
	//添加课程1
	GRect course1frame = {{34,70},{12,80}};
	display_target_layerText(p_window,&course1frame,GAlignLeft,GColorBlack,g_courses[0],U_ASCII_ARIAL_12);

	//添加课程2
	GRect course2frame = {{34,91},{12,80}};
	display_target_layerText(p_window,&course2frame,GAlignLeft,GColorWhite,g_courses[1],U_ASCII_ARIAL_12);

	//添加课程3
	GRect course3frame = {{34,111},{12,80}};
	display_target_layerText(p_window,&course3frame,GAlignLeft,GColorWhite,g_courses[2],U_ASCII_ARIAL_12);
	show2day=0;
...
//	{
	...
	GRect tmpframe;
	//绘制表格//行宽和行高   4列，最多10行
	int x0=4,y0=4,w=40,h;
	h=classcount1>9?12:120/classcount1;
	...
	for(i=0; i<classcount1; i++)
	{
		...
		display_target_layerText(p_window,&tmpframe,GAlignLeft,GColorWhite,tmp,U_ASCII_ARIAL_12);
	}
	...
	for(i=0; i<classcount1; i++)
	{
		if(course3day[1][i]!=100)
		{
			...
			tmpframe.size.h=h;
			tmpframe.size.w=w;
			display_target_layerText(p_window,&tmpframe,GAlignLeft,GColorWhite,courses[course3day[1][i]]+1,U_ASCII_ARIAL_12);
		}
	}
	//绘制网格
...
static void timer_callback(date_time_t tick_time, uint32_t millis, void *context)
{
	...
	for(i=0;i<classcount;i++)
	{
		if(classtime[i]-timenow==15)
		{
			...
			sprintf(param.main_title, "上课时间到");
			//char tmp[30];
			sprintf(param.sub_title,"%s课十五分钟后开始!",courses[today_course[i]]+1);
