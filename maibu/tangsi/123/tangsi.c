...
#define PER_ROW_MAX_CHN_CHARACTER_NUM  9
...
P_Window init_window(void)
{
	...
	/*创建背景图层*/
	GRect temporary_frame = {{0, 0}, {128, 128}};
	P_GRect temp_p_frame = &temporary_frame;
	display_target_layer(p_window,temp_p_frame,GAlignCenter,GColorWhite,BG);
	...
	temporary_frame.size.h = 30;
	temporary_frame.size.w = 110;
		
	sprintf(buffer, "%02d:%02d", t.hour, t.min);
	
	display_target_layerText(p_window,temp_p_frame,GAlignCenter,GColorBlack,buffer,U_ASCII_ARIAL_24);
	...
	LayerScroll ls1 = {{{0,30},{98,110}}, SCROLL_KEY, readed_offset, U_ASCII_ARIAL_12,2};
