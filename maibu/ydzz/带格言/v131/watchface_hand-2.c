 ...
 *
 *    Description:  运动指针  
 *    Corporation:  修改自传统指针  gliu (), gliu@damaijiankang.com
 * 
 *         Author:  qs100371 
 *      
 *
...
static uint8_t g_triangle_layer_h = 0, g_triangle_layer_m = 0,g_text_layer=0;
uint32_t g_triangle_window = 0;
uint32_t lasttime=0;
bool showtext=true;
static count=0;
...
/*显示步数文本图层*/
#define SYSINFO_STEP_ORIGIN_X		40
#define SYSINFO_STEP_ORIGIN_Y		80
#define SYSINFO_STEP_SIZE_H		16		
#define SYSINFO_STEP_SIZE_W		48

const unsigned char motto[6][50] = 
{"希望是穷人的面包","天才出自勤奋","一分耕耘,一分收获","奋发有为,时不我待","百学须先立志","高山仰止,景行行止"};

uint8_t hour_pos[][3][2] = 
{
...
void scroll_select_back(void *context)
{
	...
	//P_Window p_window = app_window_stack_get_window_by_id(g_triangle_window);
	P_Layer p_layer=NULL;
	uint32_t nowt=app_get_time_number();
...
P_Window init_triangle_watch(void)
{
	...
	app_window_add_layer(p_window, bgl);
             ...
             //app_layer_set_bg_color(ml, GColorWhite);
             //app_layer_set_bg_color(hl, GColorWhite);
             //app_layer_set_bg_color(cl1, GColorWhite);
             //app_layer_set_bg_color(cl1, GColorWhite);
               /*创建文本图层框架范围*/
	if (showtext)
	{
	GRect frametxt = {{0, 30}, {12, 128}};
	//unsigned char buf[]=motto[0];
             uint8_t index=datetime.min/10;
	/*生成文本结构体, 依次为文本内容、文本显示框架、对齐方式、字体字号*/
	LayerText text = {motto[index], frametxt, GAlignCenter, U_GBK_SIMSUN_12};

	/*创建文本图层*/
	P_Layer layer = app_layer_create_text(&text);

	/*添加文本图层到窗口中*/
	g_text_layer=app_window_add_layer(p_window, layer);
    }
	...
	LayerText lt_step = {str, frame_step, GAlignCenter, U_ASCII_ARIAL_12,1};
