...
P_Window init_window(void)
{
    ...
    char buff[100]={"wealth               n.财富，财产；丰富"};
	//uint16_t i;
	
    //maibu_read_user_file(MY_FILE_KEY, offset, buff, 100);
	memcpy(little_note_text,buff,100);
	...
	memset(buffer, 0, sizeof(buffer));
	sprintf(buffer,"%03d",line);
	display_target_layerText(p_window,temp_p_frame,GAlignLeft,g_layer1_others,buffer,U_ASCII_ARIAL_12);
     /*添加词库类型图层*/
	temporary_frame.origin.x = 50;
	temporary_frame.origin.y = 116;
	temporary_frame.size.h =12;
	temporary_frame.size.w = 78;

	display_target_layerText(p_window,temp_p_frame,GAlignRight,g_layer1_others,wordtype,U_ASCII_ARIAL_12);
