...
static void app_watch_update()
{
	...
	if(0!=maibu_get_temperature(&temp)) return;   //实时温度
	
	maibu_get_speed_info(&spinfo);   //判断是否正在运动
	...
	{
	   if((dt.mon==1)&&(dt.mday<=27))     //过年提醒
		  sprintf(presettext[MAX_TEXT-4],"%s","要过年了,记得回家看看."); 
	   else
		   sprintf(presettext[MAX_TEXT-4],"%s","记得常回家看看.");
	   if((dt.hour>=9)&&(dt.hour<=17)&&(dt.wday>=1)&&(dt.wday<=5))
          sprintf(presettext[MAX_TEXT-3],"%s","工作时间,请勿打扰.");
       else
          sprintf(presettext[MAX_TEXT-3],"%s","休息时间，好好享受.");
	  
	   if (temp<20)
		   sprintf(presettext[MAX_TEXT-2],"%s","宝宝有点冷,带我做做运动吧.");
	   else if(temp<=35)
		   sprintf(presettext[MAX_TEXT-2],"%s","现在气温正好，享受生活吧.");
	   else
	...
	else
	{
		...
		sprintf(buff,"%s%d\n","今日步数:",spdata.step);
		char tmpstr[20];
		sprintf(tmpstr,"%s%2.1f\n","实时速度:",3.6*spinfo.real_time_speed/100);
		int i,j;
		j=strlen(buff);
		for(i=0;i<strlen(tmpstr);i++)
			buff[j+i]=tmpstr[i];
		sprintf(tmpstr,"%s%2.1f","体感温度:",temp-10);
