...
enum GColor  color1 = GColorBlack;
...
void select_back(void *context)
{
  ...
  GPoint points[4] = { {3, 56}, {173, 56}, {173, 116}, {3, 116}};
  Polygon background   = {4, points};
  Geometry geometry = {GeometryTypePolygon, FillArea, color1, (void*)&background};
  ...
  sprintf(str, "This week I walked");
...
P_Window init_watch(void)
{
  ...
  struct date_time dt;
  app_service_get_datetime(&dt);
  enum GColor colors[] = {GColorBlack, GColorRed, GColorBlue, GColorPurple, GColorGreen}; //, GColorYellow, GColorCyan
  color1 = colors[dt.min%5];
  char str[20] = "";

  GRect frame_day = {{0, 16}, {26, 176}};
  sprintf(str, "%d/%d,%s", dt.mon, dt.mday, wday[dt.wday]);
  LayerText lt_day = {str, frame_day, GAlignCenter, color1, U_ASCII_ARIAL_20};
  ...
  if (inwalking)  //info.flag
    {
      ...
      LayerText lt_time = {str, frame_time, GAlignCenter, color1, U_ASCII_ARIAL_42};
  ...
  Geometry lg1 = {GeometryTypeLine, FillOutline, color1, (void*)&lh};
  geometry[num++] = &lg1;

  Line lm = {{CX, CY}, {min_pos[dt.min][0], min_pos[dt.min][1] }};
  Geometry lg2 = {GeometryTypeLine, FillOutline, color1, (void*)&lm};
  geometry[num++] = &lg2;
  ...
  if (inwalking)
    {
      ...
      Geometry lg3 = {GeometryTypeLine, FillOutline, color1, (void*)&ls};
      geometry[num++] = &lg3;
