...
static P_Window init_watch(void)
{
  ...
  //sprintf(buff, "%d", a);
      ...
      //if (dt.min % 2)
        //sprintf(buff, "%d-%d%s%s", dt.mon, dt.mday, wday_str[dt.wday], sld.festival);
      //else
        //sprintf(buff, "%s%s%s", sld.mon, sld.day, sld.solar_term); //
    
    sprintf(buff, "%d/%dm", 3721, 2567);
  ...
  //else
    sprintf(buff, "%d.%d", speed / 10, speed % 10);
      ...
      text2frame.size.w = 40;
      text3frame.origin.x = text2frame.size.w;
      text4frame.origin.x = text2frame.size.w;
   
  LayerText text2 = {buff, text2frame, GAlignLeft, GColorBlack, U_ASCII_ARIAL_24};//
  ...
  /*如果icon_type大于0，显示天气图标*/
      ...
      GBitmap bitmap_icon;
      res_get_user_bitmap(weather_get_icon_key(1), &bitmap_icon);
