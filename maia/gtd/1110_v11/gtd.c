  ...
  while (strlen(buf) == 0)   //获取到的下一节日是否为空，直到获取到下一节日不为空为止
    {
      ...
      else if ((tt.mon == 1) || (tt.mon == 3) || (tt.mon == 5) || (tt.mon == 7) || (tt.mon == 8) || (tt.mon == 10))  //小月处理
        {
          if (tt.mday == 31)
            {
              tt.mday = 1;
	  ...
	  if ((g_s_task_list.list[i].date == dt.mon*100 + dt.mday || g_s_task_list.list[i].date == 0) && ((g_s_task_list.list[i].id == 0) || (g_s_task_list.list[i].id == 1)))
        {
          ...
          if ((g_s_task_list.list[i].alarmed == 0) && (time - now <= 15) && (time - now >= 0))
          ...
          if ((g_s_task_list.list[i].date == dt.mon*100 + dt.mday) && (now - time > 10)) //(g_s_task_list.list[i].alarmed == 1)
            //如果已过期10分钟，则删除
            memset(&g_s_task_list.list[i], 0, sizeof(STaskItem));
          else if (g_s_task_list.list[i].date == 0 && now - time >= 10)
            //每日提醒，过期10分则清除已提醒标记
            g_s_task_list.list[i].alarmed = 0;
        }
      else if (g_s_task_list.list[i].id == 3)
        { //节日当天标记节日项
