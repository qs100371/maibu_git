...
void add_festival(uint8_t index)
{
  struct date_time tt;
  app_service_get_datetime(&tt);
  SLunarData lunar_data;
  char buf[50];
  maibu_get_lunar_calendar(&tt, &lunar_data);
  sprintf(buf, "%s%s", lunar_data.solar_term, lunar_data.festival);
  while (strlen(buf) == 0)   //获取到的下一节日是否为空，直到获取到下一节日不为空为止
    {
      if ((tt.mon == 4) || (tt.mon == 6) || (tt.mon == 9) || (tt.mon == 11)) //大月处理
        {
          if (tt.mday == 30)
            {
              tt.mday = 1;
      ...
      if ((tt.mon == 1) || (tt.mon == 3) || (tt.mon == 5) || (tt.mon == 7) || (tt.mon == 8) || (tt.mon == 10))  //小月处理
        {
          if (tt.mday == 31)
            {
              tt.mday = 1;
  ...
  if (index < TASK_LIST_MAX_SIZE - 1)
    {
      memset(&g_s_task_list.list[index], 0, sizeof(STaskItem));
      g_s_task_list.list[index].id = 3;
      g_s_task_list.list[index].date = tt.mon * 100 + tt.mday;
      strcpy(g_s_task_list.list[index].title, buf);
      if (index == g_s_task_list.nums)
        g_s_task_list.nums++;
    }
}
...
void timer_callback(date_time_t tick_time, uint32_t millis, void *context)
{
  ...
  for (i = 0; i < g_s_task_list.nums; i++)
    {
      if ((g_s_task_list.list[i].date == dt.mon*100 + dt.mday) && ((g_s_task_list.list[i].id == 0) || (g_s_task_list.list[i].id == 1)))
        {
          int32_t time = g_s_task_list.list[i].time / 100 * 60 + g_s_task_list.list[i].time % 100;
          if ((g_s_task_list.list[i].alarmed == 0) && (time - (dt.hour*60 + dt.min) < 15))
            //如果没有提醒过，并且是在15分钟以内，则提醒
            {
              g_s_task_list.list[i].alarmed = 1;
              memset(&param, 0, sizeof(NotifyParam));
              res_get_user_bitmap(ALARM,  &param.bmp);
              sprintf(param.main_title, "%s", "GTD提醒您");
          ...
          if ((g_s_task_list.list[i].alarmed == 1) && ((dt.hour*60 + dt.min) - time > 10))
            //如果已提醒过，并且过期10分钟，则删除
            memset(&g_s_task_list.list[i], 0, sizeof(STaskItem));
        }
      if (g_s_task_list.list[i].id == 3)
        { //节日当天标记节日项
          if (g_s_task_list.list[i].date == dt.mon*100 + dt.mday)
            g_s_task_list.list[i].alarmed = 1;
          else if (g_s_task_list.list[i].alarmed == 1)
            //替换过期的节气或节日
            //memset(&g_s_task_list.list[i], 0, sizeof(STaskItem));
            add_festival(i);
        }
              ...
              if (i - j - 13 > 0)
                {
                  ...
                  g_s_task_list.list[count].date = (buffer[j+1] - '0') * 1000 + (buffer[j+2] - '0') * 100 + (buffer[j+4] - '0') * 10 + (buffer[j+5] - '0');
                  g_s_task_list.list[count].time = (buffer[j+7] - '0') * 1000 + (buffer[j+8] - '0') * 100 + (buffer[j+10] - '0') * 10 + (buffer[j+11] - '0');

                  memcpy(g_s_task_list.list[count].title, buffer + j + 13, i - j - 13);
                }
            }
              ...
              if (i - j - 12 > 0)
                {
                  ...
                  g_s_task_list.list[count].date = (buffer[j] - '0') * 1000 + (buffer[j+1] - '0') * 100 + (buffer[j+3] - '0') * 10 + (buffer[j+4] - '0');
                  g_s_task_list.list[count].time = (buffer[j+6] - '0') * 1000 + (buffer[j+7] - '0') * 100 + (buffer[j+9] - '0') * 10 + (buffer[j+10] - '0');
                  memcpy(g_s_task_list.list[count].title, buffer + j + 12, i - j - 12);
                }
            }
          else
            {
              if (i - j > 0)
                {
                  ...
                  memcpy(g_s_task_list.list[count].title, buffer + j , i - j);
                }
            }
...
int main()
{
  ...
  app_service_timer_subscribe(60000, timer_callback, (void *)p_window);
