...
typedef struct tag_STaskItem
  {
    uint8_t id;	//任务类型ID  0 日程,1 有限定日期的待办,2 长期任务
    uint32_t date;	//任务日期
    uint32_t time;	//任务时间
    char title[24];	//任务标题
    int8_t alarmed; //0 ,1 已提醒,-1 过期
  }STaskItem;
...
const static int32_t ids[] = {ID0, ID1, ID2};
...
void timer_callback(date_time_t tick_time, uint32_t millis, void *context)
{
  ...
  for (i = 0; i < g_s_task_list.nums; i++)
    {
      if ((g_s_task_list.list[i].date == dt.mon*100 + dt.mday) && (g_s_task_list.list[i].id != 2))
        {
          ...
          if ((g_s_task_list.list[i].alarmed == 0) && (time - (dt.hour*60 + dt.min) < 15))
            //如果没有提醒过，并且是在15分钟以内，则提醒
            {
              ...
              strcpy(param.sub_title, g_s_task_list.list[i].title);
  ...
  if (g_s_task_list.nums == 0)
  ...
  else
    {
      ...
      for (; i < g_s_task_list.nums && count < 4; i++)
        {
          if (g_s_task_list.list[i].alarmed == -1)
            {
              continue;
            }
          ...
          memcpy(buff, g_s_task_list.list[i].title, sizeof(g_s_task_list.list[i].title));
          ...
          if (0 == g_s_task_list.list[i].date)	//为0则需要显示为近期
            {
              strcpy(buff, "近期");
            }
          else
