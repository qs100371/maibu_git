...
//	{
	...
	for(i=0; i<classcount1; i++)
	{
		...
		display_target_layerText(p_window,&tmpframe,GAlignLeft,GColorBlack,tmp,U_ASCII_ARIAL_16,GColorWhite);
	}
	...
	for(i=0; i<classcount1; i++)
	{
		if(course3day[1][i]!=100)
		{
			...
			tmpframe.size.h=h;
			tmpframe.size.w=w;
			display_target_layerText(p_window,&tmpframe,GAlignLeft,GColorBlack,courses[course3day[1][i]]+1,U_ASCII_ARIAL_16,GColorWhite);
		}
	}
	//绘制网格
